import { BLOG_VALIDATION, VALIDATION_MESSAGES } from "@/constants/messages";
import type { BlogFormData, FormData } from "@/types/types";
...
// Blog validation
export const validateBlog = (formData: BlogFormData) => {
    const errors: Record<string, string> = {};

    if (!formData.title.trim()) {
        errors.title = BLOG_VALIDATION.TITLE.REQUIRED
    } else if (formData.title.length > 250) {
        errors.title = BLOG_VALIDATION.TITLE.MAX_LENGTH
    }

    if (!formData.content.trim()) {
        errors.content = BLOG_VALIDATION.CONTENT.REQUIRED
    }

    if (!formData.image) {
        errors.image = BLOG_VALIDATION.IMAGE.REQUIRED
    }
